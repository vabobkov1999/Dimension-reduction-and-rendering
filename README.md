...
In this work, using python and PCA and t-SNE methods, visualize the features obtained in previous works (links to them are indicated below).
...
Цель работы
------------
С помощью python3.8 и методов PCA и t-SNE визуализировать фитчи из многомерной плоскости на двумерную плоскость, которые были получены в прошлых заданиях(см. таблицу выше).

Выполнение работы
-----------------


Для реализации поставленной задачи воспользуемся двумя методами:


Первый из них это метод PCA. Он находит наилучшую n-мерную плоскость, используется при компрессии изображений и видео, подавлении шума на изображениях и конечно визуализации данных. Работает следующим образом:
...
### Для работы №4 Clustering-Wikipedia-Articles
...
До момента #Start PCA  эта работа и работа №4 полностью совпадают.
...
С методом t-SNE идея таже, создается одноимённая модель, где параметры n_components - размерность; perplexity - минимальное расстояние между двумя точками; random_state - это константа, которая рандомно выбирает точку с которой начинает обработку.


Аналогично с помощью функции fit_transform запихиваем данные из X, то есть  вектора с фитчами в tmp, который в конечном итоге у нас уже запоминает двухмерные вектора (Фрагмент кода представлен ниже).
...
### Для работы №3 Image-Training

Для реализации этого задания необходимо два файла из работы №3: testing.py и training.py.

Файл training.py полностью совпадает с  работой №3 и предназначен для получения фитч из картинок и сохраняет результат в файл h5.

Файл testing.py содержит, изменённый под задание этой лабораторной работы, главный код.
...
В качестве доп.задания мне было предложено для работы [№3](https://github.com/vabobkov1999/Image-Training)
перестроить графики из двухмерной плоскости в трёхмерную.
...
* В ax.scatter был добавлен ещё один интервал tmp[k:n] (где k и n, некоторые числа задающие интервал необходимых фотографий в dataset)
...
Замечание:
-----------
Для лабораторной работы [№3](https://github.com/vabobkov1999/Image-Training) методы PCA и t-SNE работают ужасно. Это происходит из-за того, что некачественно были выбраны фитчи.

Вследствие чего они не подходят для кластеризации и построения графиков.Я пытался отрегулировать параметры фитч, но лучше не получилось.

#### Как должен выглядеть график методов PCA или t-SNE:
